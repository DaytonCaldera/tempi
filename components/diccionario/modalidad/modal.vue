  ...
  <v-dialog v-model="props.dialog" max-width="65vw" persistent>
    <v-card>
        ...
        <v-container>
          <v-row>
            ...
            <v-col cols="9" sm="9" md="9">
              <v-text-field v-model="props.editedItem.modalidad" label="Nombre de la modalidad"
                @change="updateData('modalidad', $event.target.value)"></v-text-field>
            </v-col>
          </v-row>
      ...
      <v-card-actions>
        ...
        <v-btn color="blue-darken-1" variant="text" @click="$emit('handleAddModalidad')">
          {{ buttonSaveTitle }}
        </v-btn>
      </v-card-actions>
    </v-card>

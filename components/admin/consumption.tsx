...
import { Download, Calendar, ArrowUpRight, ArrowDownRight, ChevronDown, ChevronUp, Package, MapPin } from "lucide-react";
...
export default function ConsumptionReport({ deptId, title = "Reporte de Consumo" }: ReportProps) {
    ...
    return (
        <div className="space-y-12">
            ...
            {/* 1. RESPONSIVE HEADER */}
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 bg-white p-6 md:p-8 rounded-4xl border border-gray-100 shadow-sm">
                <div className="">
                ...
                {/* The Container for Inputs + Button */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full xl:w-auto">

                    {/* DATE PICKER GROUP - Using flex-wrap for tight spaces */}
                    <div className="flex flex-wrap items-center gap-3 bg-gray-50 p-3 sm:px-4 sm:py-3 rounded-2xl border border-gray-100 flex-1 lg:flex-none">
                        ...
                        <div className="flex items-center gap-3 min-w-40 flex-1 sm:flex-none">
                            ...
                            <input
                                type="datetime-local"
                                className="bg-transparent text-[15px] font-bold outline-none cursor-pointer w-full"
                                style={{ minWidth: "140px" }} // Hard-coded min-width to prevent browser squishing
                                value={range.start}
                                onChange={(e) => setRange({ ...range, start: e.target.value })}
                            />
                        </div>

                        {/* Separator - Hidden when wrapped (mobile) */}
                    ...
                    {/* DOWNLOAD BUTTON */}
                    <button className="flex items-center justify-center p-4 bg-[#171717] text-white rounded-2xl hover:bg-black transition-all shadow-xl active:scale-95 shrink-0 h-full">
                        <Download size={22} />
                    </button>
                </div>
            </div>
